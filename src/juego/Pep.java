...
public class Pep {
    ...
    public Pep(int x, int y, int ancho, int alto, Color color) {
        this.x = x;
        this.y = y - alto / 2;  // Ajusta la posición inicial de y para que Pep esté sobre la isla
        this.ancho = ancho;
        this.alto = alto;
        this.color = color;
        this.enSuelo = true;  // Establece que Pep comienza en el suelo
        this.velocidadY = 0;
        this.ultimaDir = "der";  // Dirección inicial hacia la derecha
    ...
    public void dibujar(Entorno entorno) {
        ...
        if (estaQuieto()) {
            img = ultimaDir.equals("izq") ? quietoi : quietod;
        }
        // Si Pep está saltando o cayendo
        else if (!enSuelo) {
            img = ultimaDir.equals("der") ? saltod : saltoi;
        }
        // Si Pep se está moviendo en el suelo
        else {
            img = ultimaDir.equals("der") ? correrd : correri;
        }
